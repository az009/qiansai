...
void DCMI_DoubleBuffer_Start(){
    ...
    CPLD_Stop();
    CPLD_Base_Clock(1);
    HAL_StatusTypeDef ret =  HAL_DCMI_Start_DMA(&hdcmi, DCMI_MODE_CONTINUOUS, (uint32_t)raw_big_buf, RAW_BUF_WORDS * 2);
    uart1_printf("[TEST] HAL_DCMI_Start_DMA ret=%d, hdcmi.State=%d\r\n",
             ret, hdcmi.State);
...
/**
*   @brief 将对cpld采集到的数据处理放在这里
*   中断回调函数之后会触发这个任务执行
**/
void CPLD_Deal_Task(void *argument){

    while (1) {
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
        if (ready_half == 0) {
            uint8_t *buf = (uint8_t *)raw_big_buf;
            Parse_Frame(buf);
        }else if (ready_half == 1) {
            uint8_t *buf = (uint8_t *)(raw_big_buf + RAW_BUF_WORDS);
            Parse_Frame(buf);
        }      
    }
}
...
void Control_DCMI_Task(void *argument){
    while (1) {
        //如果置1表示要开启dcmi
        if (SHM_CONFIG->dcmi_enable == 1) {
            SHM_CONFIG->dcmi_enable = 2;
            DCMI_DoubleBuffer_Start();
            if (xParseTaskHandle == NULL) {
                xTaskCreate(CPLD_Deal_Task, "CPLD_Deal_Task", 512, NULL, osPriorityHigh, &xParseTaskHandle);
            }  
        }else if (SHM_CONFIG->dcmi_enable == 0){
            SHM_CONFIG->dcmi_enable = 2;
            CPLD_Stop();
            HAL_DCMI_Stop(&hdcmi);
            if (xParseTaskHandle != NULL) {
                vTaskDelete(xParseTaskHandle);
                xParseTaskHandle = NULL;
            }   
        }
