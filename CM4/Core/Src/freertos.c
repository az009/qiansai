...
SemaphoreHandle_t hsem_config_sem = NULL;     /* ISR 中 Give，Proto_Select 中 Take */
TaskHandle_t proto_select_handle = NULL;      /* 协议 Task 完成后 Notify 此句柄 */
...
void Test_DMA_Task(void *argument);
...
void MX_FREERTOS_Init(void) {
  ...
  /* add semaphores, ... */
  hsem_config_sem = xSemaphoreCreateBinary();
  ...
  //xTaskCreate(Test_DMA_Task, "Test_DMA_Task", 256, NULL, osPriorityNormal, NULL);
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
    ...
    vTaskDelay(pdMS_TO_TICKS(10));  /* SysTick 中断没触发(osDelay 卡)，临时用 HAL_Delay(TIM7 tick) */
  }
...
/* M4 协议调度任务：
 *   1. 阻塞等 hsem_config_sem（HSEM ISR 中 Give）
 *   2. 杀掉旧协议 Task（如果有），apply 新配置，创建新协议 Task
...
void Proto_Select(void *argument){
  ...
  for(;;)
  {
    ...
    if (active_proto_task != NULL) {
      if (eTaskGetState(active_proto_task) != eDeleted) {
        vTaskDelete(active_proto_task);
      }
      active_proto_task = NULL;
    }

    /* apply 新配置 + 创建新协议 Task */
    if (SHM_CONFIG->active_proto == 1) {
      ...
      xTaskCreate(UART_Callback_Task, "UART_Callback", 256, NULL,
                  osPriorityNormal, &active_proto_task);
    }
      ...
      xTaskCreate(SPI_Callback_Task, "SPI_Callback", 256, NULL,
                  osPriorityNormal, &active_proto_task);
    }
      ...
      xTaskCreate(I2C_Callback_Task, "I2C_Callback", 256, NULL,
                  osPriorityNormal, &active_proto_task);
    }
      ...
      xTaskCreate(CAN_Callback_Task, "CAN_Callback", 256, NULL,
                  osPriorityNormal, &active_proto_task);
    }
  }
}
