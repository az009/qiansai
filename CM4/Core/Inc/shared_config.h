...
#define SHM_DCMI_B_ADDR     (SHM_DCMI_ADDR + CHANNEL_SAMPLES)
#define SHM_DCMI_CTRL_ADDR  (SHM_DCMI_B_ADDR + CHANNEL_SAMPLES)
...
/* 编译期检查：超过 64 字节编译直接报错 */
_Static_assert(sizeof(proto_config_t) <= 64,
               "proto_config_t too large! Increase SHM_STATUS_ADDR offset.");

#define SHM_STATUS_ADDR  (SHM_CONFIG_ADDR + 64U)   // 留 64 字节给 proto_config_t
typedef struct {
