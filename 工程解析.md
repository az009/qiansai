...
详细对照表见 [docs/协议设置表.md](docs/协议设置表.md)。速查:
...
- 工具链:`gcc-arm-none-eabi.cmake`(STM32CubeCLT),CMake + Ninja
- 一键分模块验证(从零配置、清理重编、检查共享头唯一性和 ELF 产物):
  ```powershell
  powershell -ExecutionPolicy Bypass -File tools\verify.ps1 -Clean
  ```
- 验证产物在 `build/verify/CM4/qiansai_CM4.elf` 和 `build/verify/CM7/qiansai_CM7.elf`
- 日常开发可用 `CM4/build/`、`CM7/build/` 增量构建;发布/合并前跑上面的 verify 脚本
...
| `CM4/Modules/Protocols/{UART,SPI,I2C,CAN}/my_*_check.c` | 4 协议采集/解析模块 |
| `CM4/Modules/Protocols/Timing/my_dwt_count.c` | DWT 微秒计时 |
| `Common/Inc/shared_config.h` | 共享内存结构 + HSEM ID + REC_MAGIC(**双核唯一一份**)|
| `Common/Inc/shared_buf.h` | ring buffer 结构 + shm_push/shm_pop |
...
| `docs/协议设置表.md` | 4 协议参数对照表 |

---

## 关联文档
- [docs/协议设置表.md](docs/协议设置表.md) — 4 协议每个参数的含义
